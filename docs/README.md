...
### Exemples (pour comprendre SudachiPy, n-grammes, TF‑IDF)

On prend une phrase exemple :

> 今日は新しいプロジェクトの会議があります。
>
> Traduction : « Aujourd’hui, il y a une réunion pour le nouveau projet. »

#### A) SudachiPy : tokenisation + lemmatisation (forme dictionnaire)

Dans ce projet, SudachiPy est surtout utile pour **mieux retrouver le vocabulaire JLPT** : les listes JLPT sont souvent en **forme dictionnaire** (lemme), alors que le texte contient des **formes conjuguées**.

Exemples (idée) :

- texte : `飲みます` → lemme : `飲む`
- texte : `勉強しました` → lemme : `勉強する`
- texte : `見て` → lemme : `見る`

Avec SudachiPy, on peut obtenir ce lemme via `dictionary_form()` (ou une forme normalisée), puis faire le “lookup JLPT” sur cette forme. Sans SudachiPy, le projet retombe sur des heuristiques (dé‑conjugaison simple) + matching par sous-chaîne.

#### B) n-grammes de caractères (modèle TF‑IDF)

Un **n‑gramme de caractères** = une suite de $n$ caractères consécutifs. Exemple sur « 新しいプロジェクト » en **3‑grammes** :
...
#### C) TF‑IDF (importance d’un fragment dans un corpus)

TF‑IDF sert à donner un poids plus fort aux fragments **caractéristiques** d’un texte, et un poids plus faible à ceux qui sont **partout**.

Formule (version classique) :

$$
\mathrm{tfidf}(t, d) = \mathrm{tf}(t, d) \times \log\Big(\frac{N}{\mathrm{df}(t)}\Big)
$$

- $t$ = un fragment (ici un n‑gramme de caractères)
- $d$ = un texte
- $N$ = nombre total de textes
- $\mathrm{df}(t)$ = nombre de textes qui contiennent $t$

Exemple (corpus minuscule, $N=2$) :

- $d_1$ : 今日は新しいプロジェクトの会議があります。
- $d_2$ : 今日は会議があります。

Si un fragment lié à « 会議…あります » apparaît dans $d_1$ **et** $d_2$, alors $\mathrm{df}(t)=2$ et le facteur $\log(2/2)=0$ : ce fragment aide peu à distinguer les textes.
À l’inverse, un fragment issu de « プロジェクト » n’apparaît que dans $d_1$ (donc $\mathrm{df}(t)=1$), et son poids TF‑IDF est plus élevé (facteur $\log(2/1)$).

Point important : dans ce projet, **SudachiPy aide surtout les features “JLPT”** (matching vocabulaire/lemmes), tandis que **TF‑IDF en n‑grammes de caractères** sert à donner une “empreinte” du texte sans exiger une tokenisation parfaite.
